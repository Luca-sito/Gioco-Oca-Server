...
async function ripristinaPartiteDaFirebase() {
  ...
  for (const id in partiteFirebase) {
    ...
    } else if (partitaRipristinata.fase === "preparazione_gioco") {
      preparaAttesaClientGioco(partitaRipristinata, p.stanza);
...
function calcolaMovimento(posizioneAttuale, valoreDado) {
  ...
  function aggiungiEvento(indicePercorso, casella, messaggio) {
    const evento = {
      indicePercorso: Math.max(0, Number(indicePercorso) || 0),
      casella: Number(casella),
      messaggio
    };
    eventiPercorso.push(evento);
  ...
  if (nuovaPosizione > CASELLA_VITTORIA) {
    ...
    if (indiceTraguardo >= 0) {
      aggiungiEvento(indiceTraguardo, CASELLA_VITTORIA, "Hai superato il traguardo, rimbalzi indietro!");
    }
  ...
  const indiceAtterraggio = Math.max(0, percorso.length - 1);

  if (nuovaPosizione === CASELLA_VITTORIA) {
    vittoria = true;
    aggiungiEvento(indiceAtterraggio, nuovaPosizione, "🎉 Hai vinto!");
    return { nuovaPosizione, percorso, messaggi, eventiPercorso, turniDaSaltare, vittoria, tiraAncora };
  }

  if (nuovaPosizione === CASELLA_TIRA_ANCORA) {
    tiraAncora = true;
    aggiungiEvento(indiceAtterraggio, nuovaPosizione, "Sali sul ponte! Tira ancora i dadi.");
  }

  if (CASELLE_AVANZA_ANCORA.includes(nuovaPosizione)) {
    aggiungiEvento(indiceAtterraggio, nuovaPosizione, "Avanzi dello stesso numero di caselle!");
    ...
    return {
      ...
      eventiPercorso: eventiPercorso.concat((r.eventiPercorso || []).map(evento => ({
        ...evento,
        indicePercorso: Number(evento.indicePercorso || 0) + offset
      }))),
  ...
  if (CASELLE_SALTA_TRE_TURNI.includes(nuovaPosizione)) {
    turniDaSaltare = 3;
    aggiungiEvento(indiceAtterraggio, nuovaPosizione, "Rimani fermo per 3 turni!");
  }

  if (CASELLE_SALTA_UN_TURNO.includes(nuovaPosizione)) {
    turniDaSaltare = 1;
    aggiungiEvento(indiceAtterraggio, nuovaPosizione, "Salti un turno!");
  }

  if (CASELLE_TORNA_A[nuovaPosizione] !== undefined) {
    const casellaSpeciale = nuovaPosizione;
    const cf = CASELLE_TORNA_A[nuovaPosizione];
    aggiungiEvento(indiceAtterraggio, casellaSpeciale, `Torni alla casella ${cf}!`);
    percorso.push(cf);
    nuovaPosizione = cf;
  }
...
const DURATA_ANIMAZIONE_DADI_SERVER_MS = 1200;
const DURATA_PASSO_PEDINA_SERVER_MS = 420;
const ATTESA_MASSIMA_CLIENT_GIOCO_MS = 12000;

function durataAnimazioneMossaMs(percorso) {
  const passi = Array.isArray(percorso) ? percorso.length : 0;
  return DURATA_ANIMAZIONE_DADI_SERVER_MS + passi * DURATA_PASSO_PEDINA_SERVER_MS;
}

function tempoRimanenteTurnoMs(partita) {
  if (!partita || !partita.scadenzaTurno) return millisecondiMossa(partita);
  return Math.max(0, Math.min(millisecondiMossa(partita), partita.scadenzaTurno - Date.now()));
}
      ...
      Object.values(partita.giocatori).forEach(g => {
        if (g.socket && g.socket.readyState === WebSocket.OPEN) {
          g.socket.send(JSON.stringify({
            ...
            percorso: risultato.percorso, eventiPercorso: risultato.eventiPercorso || [], idGiocatoreCheHaTirato: idGiocatore, automatico: !!automatico,
            messaggi: messaggiFinali,
            messaggiImmediati: automatico ? ["⏱️ Tempo scaduto: mossa automatica."] : [],
      ...
      Object.values(partita.giocatori).forEach(g => {
        if (g.socket && g.socket.readyState === WebSocket.OPEN) {
          g.socket.send(JSON.stringify({
            ...
            tempoRimanenteMs: tempoRimanenteTurnoMs(partita),
            chatAttiva: partita.chatAttiva !== false,
            mediaAttiva: partita.mediaAttiva === true
          }));
        }
    ...
    Object.values(partita.giocatori).forEach(g => {
      if (g.socket && g.socket.readyState === WebSocket.OPEN) {
        g.socket.send(JSON.stringify({
          ...
          tempoInizioTurno: partita.tempoInizioTurno, durataMossaMs: millisecondiMossa(partita), tempoRimanenteMs: tempoRimanenteTurnoMs(partita),
          chatAttiva: partita.chatAttiva !== false,
          mediaAttiva: partita.mediaAttiva === true
        }));
      }
...
function inviaStatoDeterminazione(partita, nomeStanza) {
  ...
  const messaggio = JSON.stringify({
    ...
    tempoInizioTurno: partita.tempoInizioTurno || null,
    durataMossaMs: millisecondiMossa(partita),
    tempoRimanenteMs: partita.tempoInizioTurno ? Math.max(0, Math.min(millisecondiMossa(partita), partita.tempoInizioTurno + millisecondiMossa(partita) - Date.now())) : millisecondiMossa(partita),
    chatAttiva: partita.chatAttiva !== false,
    mediaAttiva: partita.mediaAttiva === true
  });
...
function fermaAttesaClientGioco(partita) {
  if (!partita) return;
  if (partita.timerPreparazioneGioco) {
    clearTimeout(partita.timerPreparazioneGioco);
    partita.timerPreparazioneGioco = null;
  }
}

function preparaAttesaClientGioco(partita, nomeStanza) {
  fermaTimerTurno(partita);
  fermaAttesaClientGioco(partita);
  partita.fase = "preparazione_gioco";
  partita.iniziata = false;
  partita.tempoInizioTurno = null;
  partita.scadenzaTurno = null;
  partita.clientGiocoPronti = new Set();

  // Fallback: se un client non riesce a caricare la pagina, la partita non resta
  // bloccata per sempre. Normalmente il timer viene cancellato appena tutti i
  // client hanno eseguito riprendiPartita dalla pagina gioco.html.
  partita.timerPreparazioneGioco = setTimeout(() => {
    if (partita.fase !== "preparazione_gioco") return;
    iniziaFaseDeterminazione(partita, nomeStanza).catch(errore => {
      console.error("Errore avvio determinazione dopo attesa client:", errore);
    });
  }, ATTESA_MASSIMA_CLIENT_GIOCO_MS);
}

async function registraClientGiocoPronto(partita, nomeStanza, uid, socket) {
  if (partita.fase !== "preparazione_gioco") return false;
  if (!(partita.clientGiocoPronti instanceof Set)) partita.clientGiocoPronti = new Set();
  partita.clientGiocoPronti.add(uid);

  const tuttiUid = Object.keys(partita.giocatori || {});
  const tuttiPronti = tuttiUid.length >= 2 && tuttiUid.every(id => partita.clientGiocoPronti.has(id));

  if (!tuttiPronti) {
    socket.send(JSON.stringify({
      tipo: "preparazionePartita",
      pronti: partita.clientGiocoPronti.size,
      totali: tuttiUid.length,
      messaggio: "Attesa degli altri giocatori..."
    }));
    return true;
  }
...
async function avviaPartitaAutomaticamente(partita) {
  ...
  // Prima mandiamo tutti nella pagina di gioco; il countdown “CHI INIZIA?”
  // partirà soltanto quando i client gioco sono realmente collegati.
  preparaAttesaClientGioco(partita, nomeStanza);
...
inviaAllaStanza(stanzaAttuale, {
  ...
  tipoDispositivo,
        ...
        if (partita.fase === "preparazione_gioco") {
          await registraClientGiocoPronto(partita, nomeStanza, uid, socket);
          return;
        }
