...
async function concludiPartita(
  ...
  try {
    ...
    const nomeVincitore =
      (partecipanti.find((p) => p.uid === vincitoreUid) || {})
        .nome || null;
    ...
    for (const p of partecipanti) {
      ...
      if (p.uid === vincitoreUid) {
        ...
        const aggiornamenti = {
          ...
          streakVittorieMassima: Math.max(
...
async function generaNicknameGoogleUnico(
  ...
  let base = preparaNicknameGoogle(
    nome,
    email
  );
    ...
    new GoogleStrategy(
      ...
      async (
        ...
        try {
          ...
          const uid = novoRef = nuovoRef.key;
...
function inviaListaPartite(
  ...
  const lista =
    Object.values(
      ...
      // Questa proprietà deve restare presente.
      iniziata:
        p.iniziata !== false,
...
); // CHIUSURA CORRETTA DI wss.on("connection", ...)
