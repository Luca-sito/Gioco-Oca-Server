...
// ===== LOG CHAT PER MODERAZIONE: CONSERVAZIONE 90 GIORNI =====
const DURATA_LOG_CHAT_MS = 90 * 24 * 60 * 60 * 1000;
const INTERVALLO_PULIZIA_LOG_CHAT_MS = 12 * 60 * 60 * 1000;

function normalizzaUidLog(uid) {
  return String(uid || "").replace(/[.#$/\[\]]/g, "_").substring(0, 128);
}

async function salvaLogChatModerazione({
  uid,
  nickname,
  testo,
  tipoChat,
  ...
  const uidPulito = normalizzaUidLog(uid);
  if (!uidPulito) return;

  const ref = db.ref(`chatLogs/${uidPulito}`).push();
  await ref.set({
    id: ref.key,
    uid: String(uid),
    nickname: pulisciTesto(String(nickname || "Utente"), 30),
    testo: pulisciTesto(String(testo || ""), 300),
    tipoChat: tipoChat === "partita" ? "partita" : "lobby",
    stanza: stanza ? pulisciTesto(String(stanza), 50) : null,
    partitaId: partitaId ? pulisciTesto(String(partitaId), 128) : null,
    timestamp: Date.now()
  });
}

async function eliminaLogChatScadutiPerUid(uid) {
  if (!db || !uid) return 0;
  ...
  snap.forEach(figlio => {
    aggiornamenti[figlio.key] = null;
    rimossi++;
  });

  if (rimossi > 0) {
    await db.ref(`chatLogs/${uidPulito}`).update(aggiornamenti);
  }

  return rimossi;
}

async function ottieniLogChatUltimi90Giorni(uid) {
  if (!db || !uid) return [];
  ...
  snap.forEach(figlio => {
    const valore = figlio.val() || {};
    risultati.push({
      id: figlio.key,
      uid: String(valore.uid || uid),
      nickname: pulisciTesto(String(valore.nickname || "Utente"), 30),
      testo: pulisciTesto(String(valore.testo || ""), 300),
      tipoChat: valore.tipoChat === "partita" ? "partita" : "lobby",
      stanza: valore.stanza || null,
      partitaId: valore.partitaId || null,
      timestamp: Number(valore.timestamp) || 0
    });
  });

  risultati.sort((a, b) => a.timestamp - b.timestamp);
  return risultati;
}

async function pulisciTuttiLogChatScaduti() {
  if (!db) return;

  try {
    const snapUtenti = await db.ref("chatLogs").once("value");
    if (!snapUtenti.exists()) return;

    const operazioni = [];
    snapUtenti.forEach(figlio => {
      operazioni.push(eliminaLogChatScadutiPerUid(figlio.key));
    });

    const risultati = await Promise.allSettled(operazioni);
    const totale = risultati.reduce((somma, risultato) => {
      return somma + (risultato.status === "fulfilled" ? Number(risultato.value || 0) : 0);
    }, 0);

    if (totale > 0) {
      console.log(`Pulizia log chat: rimossi ${totale} messaggi più vecchi di 90 giorni.`);
    }
  } catch (errore) {
    console.error("Errore pulizia log chat:", errore);
  }
}

// Pulizia periodica. unref() evita che il timer tenga vivo il processo Node da solo.
const timerPuliziaLogChat = setInterval(
  pulisciTuttiLogChatScaduti,
...
app.post("/api/segnalazioni", limiteSegnalazioni, richiediAuth, async (req, res) => {
  ...
  try {
    ...
    let logChatSnapshot = null;
    let numeroMessaggiChat = 0;
    let periodoLogChat = null;

    if (categoria === "Chat") {
      const log = await ottieniLogChatUltimi90Giorni(uidSegnalato);
      numeroMessaggiChat = log.length;
      periodoLogChat = {
        da: Date.now() - DURATA_LOG_CHAT_MS,
        a: Date.now(),
        giorni: 90
      };

      // Copia immutabile dei log presenti al momento della segnalazione.
      // Rimane nel report anche quando i normali chatLogs vengono eliminati.
      logChatSnapshot = {};
      for (const messaggio of log) {
        logChatSnapshot[messaggio.id] = messaggio;
      }
    }
    ...
    if (categoria === "Chat") {
      datiSegnalazione.numeroMessaggiChat = numeroMessaggiChat;
      datiSegnalazione.periodoLogChat = periodoLogChat;
      datiSegnalazione.logChatSnapshot = logChatSnapshot;
    }

    await nuovaSegnalazione.set(datiSegnalazione);

    return res.json({
      ok: true,
      id: nuovaSegnalazione.key,
      numeroMessaggiChat: categoria === "Chat" ? numeroMessaggiChat : undefined
    });
  } catch (errore) {
...
// Elenco report per amministratori.
app.get("/api/admin/segnalazioni", richiediAdmin, async (req, res) => {
  if (!db) return res.status(500).json({ errore: "Servizio non disponibile." });

  try {
    const snap = await db.ref("segnalazioni").orderByChild("data").once("value");
    const segnalazioni = [];

    snap.forEach(figlio => {
      segnalazioni.push(figlio.val());
    });

    segnalazioni.sort((a, b) => Number(b?.data || 0) - Number(a?.data || 0));
    return res.json({ segnalazioni });
  } catch (errore) {
    console.error("Errore lettura segnalazioni admin:", errore);
    return res.status(500).json({ errore: "Errore durante la lettura delle segnalazioni." });
  }
});
...
async function inviaChatLobbyFiltrata(nomeStanza, mittenteUid, messaggio) {
  ...
  try {
    await salvaLogChatModerazione({
      uid: mittenteUid,
      nickname: messaggio?.nome,
      testo: messaggio?.testo,
      tipoChat: "lobby",
      stanza: nomeStanza
    });
  } catch (errore) {
    // Un problema di logging non deve bloccare la chat in tempo reale.
    console.error("Errore salvataggio log chat Lobby:", errore);
  }
...
async function inviaChatPartitaFiltrata(partita, mittenteUid, messaggio) {
  ...
  try {
    await salvaLogChatModerazione({
      uid: mittenteUid,
      nickname: messaggio?.nome || partita.giocatori?.[mittenteUid]?.nome,
      testo: messaggio?.testo,
      tipoChat: "partita",
      stanza: partita.stanza || null,
      partitaId: partita.id || null
    });
  } catch (errore) {
    // Un problema di logging non deve bloccare la chat in tempo reale.
    console.error("Errore salvataggio log chat Partita:", errore);
  }
