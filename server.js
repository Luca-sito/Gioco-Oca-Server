...
const limiteMessaggiPrivati = rateLimit({
  ...
  max: 20,
  message: {
    errore: "Stai inviando messaggi troppo velocemente, rallenta un po'."
  }
});
...
const limiteAmici = rateLimit({
  ...
  max: 30,
  message: {
    errore: "Troppe richieste di amicizia in poco tempo, rallenta un po'."
  }
});
...
function calcolaLivello(xp) {
  ...
  return {
    livello,
    sogliaAttuale: SOGLIE_LIVELLO[livello - 1],
    sogliaProssima:
...
async function concludiPartita(
  ...
  try {
    ...
    const nomeVincitore =
      (partecipanti.find(p => p.uid === vincitoreUid) || {}).nome ||
      null;
    ...
    for (const p of partecipanti) {
      ...
      const u = (
        await db.ref("utenti/" + p.uid).once("value")
      ).val();
      ...
      if (p.uid === vincitoreUid) {
        ...
        if (
          durataSecondi !== null &&
          (
            u.vittoriaPiuVeloceSecondi == null ||
            durataSecondi < u.vittoriaPiuVeloceSecondi
          )
        ) {
...
function estraiTokenDaCookieHeader(cookieHeaderGrezzo) {
  ...
  for (const parte of parti) {
    ...
    if (parte.substring(0, idx) === "token") {
...
async function statoAmicizia(mioUid, altroUid) {
  ...
  const [
    ...
    db
      .ref(`utenti/${mioUid}/amici/${altroUid}`)
      .once("value"),
    db
      .ref(`utenti/${mioUid}/richiesteInviate/${altroUid}`)
      .once("value"),
    db
      .ref(`utenti/${mioUid}/richiesteRicevute/${altroUid}`)
      .once("value")
  ]);
...
app.post("/api/registrati", limiteLogin, async (req, res) => {
  if (!db) {
    return res.status(500).json({
      errore: "Servizio account non disponibile al momento."
    });
  }
  ...
  try {
    ...
    const nicknamePulito = pulisciTesto(
      nickname,
      20
    );
    ...
    const emailPulita =
      pulisciTesto(email, 100).toLowerCase();
    ...
    if (
      await trovaUtentePerEmail(emailPulita)
    ) {
      return res.status(400).json({
        errore:
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (stato === "richiesta_inviata") {
        return res.json({
          ok: true
        });
      }

      if (stato === "richiesta_ricevuta") {
        return res.status(400).json({
          errore:
...
// ===== DADO VERO DA RANDOM.ORG =====

// timeout ridotto a 1.5s: prima erano 4s,
// troppi per una fase come "chi inizia?"
...
function costruisciStatoGiocatori(partita) {
  return partita.ordineGiocatori.map(
    id => ({
      id,
      nome:
        partita.giocatori[id].nome,
      avatar:
        partita.giocatori[id].avatar ||
        null,
      posizione:
...
function inviaListaPartite(nomeStanza) {
  ...
  const lista =
    Object.values(
      ...
      // NUOVO:
      // permettono alla lobby di sapere
      // se una partita è già iniziata
      // e se può quindi essere ripresa.
      iniziata:
        !!p.iniziata,
...
// NUOVO:
// registra il socket corrente come utente online
// nella stanza indicata.
//
// Questo è importante quando un giocatore:
// - rientra nella lobby;
// - ricarica la pagina;
// - riprende una partita;
// - apre un nuovo WebSocket.

function registraPresenzaNellaStanza(
  nomeStanza
) {
  if (
    !nomeStanza ||
...
function resultadoMessaggi(risultato) {
...
async function completaDeterminazione(
  ...
  const risultatoMovimento =
    calcolaMovimento(
      primoGiocatore.posizione,
      primoValore
    );

  primoGiocatore.posizione =
    risultatoMovimento.nuovaPosizione;

  if (
    risultatoMovimento.turniDaSaltare >
    0
  ) {
    primoGiocatore.turniSaltati =
      risultatoMovimento.turniDaSaltare;
  }

  if (
    !risultatoMovimento.tiraAncora &&
    !risultatoMovimento.vittoria
  ) {
    passaAlProssimoTurno(
      partita
    );
      ...
      g.socket.send(
        JSON.stringify({
          ...
          primoMovimento: {
            ...
            percorso:
              risultatoMovimento.percorso,

            messaggi:
              risultatoMovimento.messaggi
          },
      ...
      async message => {
        try {
          ...
          if (
            ...
            // MODIFICATO:
            // prima veniva impostata stanzaAttuale
            // prima ancora di aver verificato
            // il giocatore. Ora prima verifichiamo
            // che appartenga davvero alla partita.
            const mioGiocatore =
              partita.giocatori[
                uid
              ];
            ...
            // MODIFICATO:
            // il giocatore che riprende la partita
            // viene registrato di nuovo nella stanza.
            registraPresenzaNellaStanza(
              nomeStanza
            );
          ...
          if (
            ...
            // Registriamo la presenza nella stanza
            // anche quando si accetta un invito.
            registraPresenzaNellaStanza(
              nomeStanza
            );
      ...
      () => {
        try {
          ...
          for (
            ...
            // Le partite già iniziate NON vengono cancellate
            // quando il giocatore chiude il browser.
            // Questo permette di riprenderle in seguito.
